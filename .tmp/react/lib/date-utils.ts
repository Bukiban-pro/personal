/**
 * Date Utilities — Formatting, relative time, and date helpers.
 *
 * Stolen from: Bookverse formatDate (7 styles), ChefKix formatShortTimeAgo,
 * 5TProMart isToday/formatDateRange.
 *
 * Dependencies: None (uses native Intl.DateTimeFormat)
 *
 * @example
 * formatDate(new Date(), 'short')      // "Apr 15"
 * formatDate('2024-01-15', 'relative') // "3 months ago"
 * formatShortTimeAgo(new Date())       // "just now"
 * isToday(new Date())                  // true
 */

export type DateFormatStyle = 'short' | 'medium' | 'long' | 'full' | 'relative' | 'datetime' | 'time'
 ...
 * - 'relative': "2 hours ago", "Yesterday", falls back to 'short'
 ...
 * Stolen from: ChefKix feed timestamps.
 */
