...
/**
 * Toast — Compact, whisper-quiet notification.
 * Tiny colored dot indicator instead of heavy icons.
 * Close button hidden until hover. Semi-transparent backdrop blur.
 *
 * Dependencies: class-variance-authority, lucide-react, cn()
 *
 * @example
	...
	({ className, variant = 'default', title, description, children, action, onClose, duration = 5000, ...props }, ref) => {
		...
		return (
			...
			>
				...
				{action && (
					<button type='button' onClick={action.onClick} className='flex-shrink-0 font-medium text-primary hover:underline'>
						{action.label}
					</button>
				)}
				{onClose && (
