/**
 * PulsatingButton
 * Button with a soft pulsing glow ring or ripple effect on its outline.
 * Two variants:
 *   - "pulse"  → box-shadow pulses in/out smoothly
 *   - "ripple" → single shot ripple expands and fades
 *
 * Add @keyframes to globals.css:
 *   @keyframes pulse {
 *     0%, 100% { box-shadow: 0 0 0 0 var(--pulse-color, oklch(from var(--bg) l c h / 0.5)); }
 *     50%       { box-shadow: 0 0 0 var(--distance) var(--pulse-color, oklch(from var(--bg) l c h / 0.5)); }
 *   }
 *   @keyframes pulse-ripple {
 *     0%   { box-shadow: 0 0 0 0 oklch(from var(--pulse-color, var(--bg)) l c h / 1); }
 *     100% { box-shadow: 0 0 0 var(--distance) oklch(from var(--pulse-color, var(--bg)) l c h / 0); }
 *   }
 ...
 *   <PulsatingButton variant="ripple" pulseColor="#6366f1" duration="2s">
 *     Go Live
 *   </PulsatingButton>
 */
"use client"
...
export const PulsatingButton = React.forwardRef<HTMLButtonElement, PulsatingButtonProps>(
  ({ className, children, pulseColor, duration = "1.5s", distance = "8px", variant = "pulse", ...props }, ref) => {
    ...
    useLayoutEffect(() => {
      ...
      for (const eventName of syncEvents) button.addEventListener(eventName, scheduleBgUpdate)

      return () => {
        if (animationFrameId) window.cancelAnimationFrame(animationFrameId)
        themeObserver.disconnect()
        buttonObserver.disconnect()
        for (const eventName of syncEvents) button.removeEventListener(eventName, scheduleBgUpdate)
      }
    }, [pulseColor])
