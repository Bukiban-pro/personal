...
export interface ProtocolEngineMode {
  title: string;
  trigger?: string;
  rules?: string[];
  outputs?: string[];
...
export interface LandingProductProtocolEngineProps extends React.HTMLAttributes<HTMLElement> {
  title?: string;
  description?: string;
  modes: ProtocolEngineMode[];
}

export const LandingProductProtocolEngine = React.forwardRef<HTMLElement, LandingProductProtocolEngineProps>(
  ({ className, title = "Turn protocol into an engine with visible rules and outputs", description, modes, ...props }, ref) => {
            ...
            <div className="rounded-3xl border border-border bg-card p-6 shadow-sm lg:p-8">
              {activeMode ? (
                <>
                    ...
                    <div>
                      ...
                      <div className="mt-3 grid gap-3">
                        {(activeMode.rules || []).map((rule) => (
                          <div key={rule} className="rounded-2xl border border-border bg-muted/30 px-4 py-3 text-sm text-muted-foreground">
                            {rule}
                          </div>
                        ))}
                      </div>
                  ...
                  {activeMode.failover ? <div className="mt-6 rounded-2xl border border-border bg-primary/5 px-5 py-4 text-sm font-medium">{activeMode.failover}</div> : null}
                </>
              ) : (
                <div className="text-sm text-muted-foreground">Add modes to populate the protocol engine.</div>
              )}
            </div>
          </div>
        </div>
...
LandingProductProtocolEngine.displayName = "LandingProductProtocolEngine";