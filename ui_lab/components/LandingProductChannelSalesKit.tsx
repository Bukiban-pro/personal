...
export interface ChannelSalesAsset {
  title: string;
  format?: string;
  audience?: string;
  description?: string;
...
export interface LandingProductChannelSalesKitProps extends React.HTMLAttributes<HTMLElement> {
  title?: string;
  description?: string;
  assets: ChannelSalesAsset[];
}

export const LandingProductChannelSalesKit = React.forwardRef<HTMLElement, LandingProductChannelSalesKitProps>(
  ({ className, title = "Arm channel teams with reusable sales material", description, assets, ...props }, ref) => {
    return (
...
LandingProductChannelSalesKit.displayName = "LandingProductChannelSalesKit";