/**
 * CodeComparison
 * Side-by-side syntax-highlighted code diff viewer with shiki.
 * Supports diff notation (+/-), focus highlights, and dark/light themes.
 *
 * Deps: shiki, @shikijs/transformers, next-themes, lucide-react
 * Usage:
 *   <CodeComparison
...
export function CodeComparison({
  ...
  const renderCode = (code: string, highlighted: string) => {
    if (highlighted) {
      ...
      return <pre className="bg-background text-foreground h-full overflow-auto p-4 font-mono text-xs break-all">{code}</pre>
    }
  }
        ...
        <div className="relative grid md:grid-cols-2">
          <div
            className={cn(
              ...
              hasLeftFocus && "[&>div>pre>code>:not(.focused)]:opacity-50! [&>div>pre>code>:not(.focused)]:blur-[0.095rem]!",
          ...
          <div
            className={cn(
              ...
              hasRightFocus && "[&>div>pre>code>:not(.focused)]:opacity-50! [&>div>pre>code>:not(.focused)]:blur-[0.095rem]!",
              "[&>div>pre>code>:not(.focused)]:transition-all [&>div>pre>code>:not(.focused)]:duration-300"
            )}
          >
            <div className="border-primary/20 bg-accent text-foreground flex items-center border-b p-2 text-sm">
