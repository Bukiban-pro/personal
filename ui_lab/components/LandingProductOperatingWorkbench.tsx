...
export interface OperatingWorkbenchPanel {
  title: string;
  prompt?: string;
  constraints?: string[];
  outputs?: string[];
...
export interface LandingProductOperatingWorkbenchProps extends React.HTMLAttributes<HTMLElement> {
  title?: string;
  description?: string;
  panels: OperatingWorkbenchPanel[];
}

export const LandingProductOperatingWorkbench = React.forwardRef<HTMLElement, LandingProductOperatingWorkbenchProps>(
  ({ className, title = "Make operating design feel like a workbench, not a static blueprint", description, panels, ...props }, ref) => {
        ...
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-6">
          ...
          <div className="grid gap-6 lg:grid-cols-[0.34fr_0.66fr]">
            ...
            <div className="rounded-3xl border border-border bg-card p-6 shadow-sm lg:p-8">
              {activePanel ? (
                <>
                  <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">{activePanel.owner || "Owner"}</div>
                  <h3 className="mt-3 text-2xl font-semibold tracking-tight md:text-3xl">{activePanel.title}</h3>
                  {activePanel.prompt ? <p className="mt-4 text-sm leading-7 text-muted-foreground md:text-base">{activePanel.prompt}</p> : null}
                  <div className="mt-6 grid gap-6 xl:grid-cols-2">
                    <div>
                      <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Constraints</div>
                      <div className="mt-3 grid gap-3">
                        {(activePanel.constraints || []).map((constraint) => (
                ...
                <div className="text-sm text-muted-foreground">Add panels to populate the operating workbench.</div>
              )}
            </div>
          </div>
        </div>
...
LandingProductOperatingWorkbench.displayName = "LandingProductOperatingWorkbench";