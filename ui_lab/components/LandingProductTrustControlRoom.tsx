...
export interface TrustControlRoomView {
  title: string;
  status?: string;
...
export interface LandingProductTrustControlRoomProps extends React.HTMLAttributes<HTMLElement> {
  title?: string;
  description?: string;
  views: TrustControlRoomView[];
}

export const LandingProductTrustControlRoom = React.forwardRef<HTMLElement, LandingProductTrustControlRoomProps>(
  ({ className, title = "Turn trust posture into a live control-room view", description, views, ...props }, ref) => {
              ...
              {activeView ? (
                <>
                  ...
                  <div className="mt-6 grid gap-3">
                    {(activeView.watchpoints || []).map((item) => (
                ...
                <div className="text-sm text-muted-foreground">Add views to populate the trust control room.</div>
              )}
            </div>
          </div>
        </div>
...
LandingProductTrustControlRoom.displayName = "LandingProductTrustControlRoom";