 ...
 *
 * Stolen from: Bookverse storage.ts (typed storage service with quota handling),
 * 5TProMart storage.ts (isStorageAvailable test, generic get/set with auto JSON parse).
 *
 * Dependencies: None
 *
 * @example
 * // Define your app's storage keys
 * const KEYS = {
 *   TOKEN: 'access_token',
 *   THEME: 'theme',
 *   DRAFT: 'form_draft',
 * } as const
 *
 * setStorageItem(KEYS.TOKEN, 'abc123')
 * getStorageItem(KEYS.TOKEN) // 'abc123'
 * getStorageJson<FormDraft>(KEYS.DRAFT) // parsed object or null
 * setStorageJson(KEYS.DRAFT, { title: 'Hello' })
