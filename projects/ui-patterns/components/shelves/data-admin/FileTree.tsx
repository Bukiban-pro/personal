/**
 * FileTree
 * Animated file/folder tree with expand/collapse, selection, sorting, and
 * RTL support. Built on Radix Accordion.
 *
 * Deps: @radix-ui/react-accordion, lucide-react, @/components/ui/button,
 ...
 *   // Manual composition
 *   <Tree className="h-64">
 *     <Folder element="components" value="c1">
 *       <File value="f1"><span>Button.tsx</span></File>
 *       <File value="f2"><span>Card.tsx</span></File>
 *     </Folder>
 *   </Tree>
 */
"use client"
...
const Tree = forwardRef<HTMLDivElement, TreeViewProps>(
  (
    ...
    return (
      <TreeContext.Provider value={{ selectedId, expandedItems, handleExpand, selectItem, setExpandedItems, indicator, openIcon, closeIcon, direction }}>
        <div className={cn("size-full", className)}>
          <ScrollArea ref={ref} className="relative h-full px-2" dir={dir as Direction}>
            <AccordionPrimitive.Root {...props} type="multiple" value={expandedItems} className="flex flex-col gap-1" dir={dir as Direction}>
              {treeChildren}
    ...
    return (
      <AccordionPrimitive.Item ref={ref} {...props} value={value} className="relative h-full overflow-hidden">
        <AccordionPrimitive.Trigger
          ...
          onClick={() => { selectItem(value); handleExpand(value) }}
        >
        ...
        <AccordionPrimitive.Content className="data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down relative h-full overflow-hidden text-sm">
          ...
          <AccordionPrimitive.Root dir={direction} type="multiple" className="ml-5 flex flex-col gap-1 py-1 rtl:mr-5" value={expandedItems}>
            {children}
          </AccordionPrimitive.Root>
...
>(({ value, className, handleSelect, onClick, isSelectable = true, isSelect, fileIcon, children, ...props }, ref) => {
  ...
  return (
    <button
      ...
      onClick={(event) => { selectItem(value); handleSelect?.(value); onClick?.(event) }}
      {...props}
    >
...
>(({ className, elements, expandAll = false, children, ...props }, ref) => {
  ...
  const expendAllTree = useCallback((elements: TreeViewElement[]) => {
    ...
    for (const element of elements) expandTree(element)
...
export { CollapseButton, File, Folder, Tree, Tree as FileTree, type TreeViewElement }
