/**
 * TypingAnimation (Magic UI enhanced version)
 * Full typewriter with multi-word cycling, delete phase, configurable cursor,
 * view-triggered start, and polymorphic `as` prop. Supersedes basic TypingEffect.
 *
 ...
 *   <TypingAnimation className="text-4xl font-bold">Hello World</TypingAnimation>
 *
 *   // Multi-word cycling (types each, deletes, types next)
 *   <TypingAnimation words={["Developer", "Designer", "Builder"]} loop />
 *
 *   // Block cursor, no loop, start on view
 *   <TypingAnimation cursorStyle="block" startOnView>
...
export function TypingAnimation({
  ...
  return (
    ...
    >
      {displayedText}
      {shouldShowCursor && (
        <span className={cn("inline-block", blinkCursor && "animate-blink-cursor")}>{cursorChar}</span>
      )}
    </MotionComponent>
  )
}
