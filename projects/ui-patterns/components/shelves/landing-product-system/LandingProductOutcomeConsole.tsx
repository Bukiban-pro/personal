...
export interface OutcomeConsoleView {
  title: string;
  summary?: string;
...
export interface LandingProductOutcomeConsoleProps extends React.HTMLAttributes<HTMLElement> {
  title?: string;
  description?: string;
  views: OutcomeConsoleView[];
}

export const LandingProductOutcomeConsole = React.forwardRef<HTMLElement, LandingProductOutcomeConsoleProps>(
  ({ className, title = "Turn outcomes into a console of proof, gaps, and next moves", description, views, ...props }, ref) => {
          ...
          <div className="grid gap-6 lg:grid-cols-[0.32fr_0.68fr]">
            ...
            <div className="rounded-3xl border border-border bg-card p-6 shadow-sm lg:p-8">
              {activeView ? (
                ...
                <div className="text-sm text-muted-foreground">Add views to populate the outcome console.</div>
              )}
            </div>
          </div>
        </div>
...
LandingProductOutcomeConsole.displayName = "LandingProductOutcomeConsole";