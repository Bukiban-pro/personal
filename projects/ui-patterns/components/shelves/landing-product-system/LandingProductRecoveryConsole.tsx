...
export interface RecoveryConsoleView {
  title: string;
  severity?: string;
  checks?: string[];
  responses?: string[];
...
export interface LandingProductRecoveryConsoleProps extends React.HTMLAttributes<HTMLElement> {
  title?: string;
  description?: string;
  views: RecoveryConsoleView[];
}

export const LandingProductRecoveryConsole = React.forwardRef<HTMLElement, LandingProductRecoveryConsoleProps>(
  ({ className, title = "Elevate recovery into a console of checks and responses", description, views, ...props }, ref) => {
          ...
          <div className="grid gap-6 lg:grid-cols-[0.32fr_0.68fr]">
            ...
            <div className="rounded-3xl border border-border bg-card p-6 shadow-sm lg:p-8">
              {activeView ? (
                <>
                  ...
                  <div className="mt-6 grid gap-6 xl:grid-cols-2">
                    ...
                    <div>
                      ...
                      <div className="mt-3 grid gap-3">
                        {(activeView.responses || []).map((response) => (
                ...
                <div className="text-sm text-muted-foreground">Add views to populate the recovery console.</div>
              )}
            </div>
          </div>
        </div>
...
LandingProductRecoveryConsole.displayName = "LandingProductRecoveryConsole";