...
- **Authentication**: Zustand-based auth with persistent sessions, server-side token validation, `useAuth` hook, centralized constants
- **OTP Verification**: Email OTP flow for sign-up with dedicated page and service functions
- **Google OAuth**: Frontend integrated with `@react-oauth/google`
- **CI/CD**: GitHub Actions workflow (lint, type-check, test, build)
- **Quality**: Husky pre-commit hooks running Prettier + ESLint
- **Testing**: Jest + React Testing Library (auth service tests)
- **State Management**: Zustand stores with persist middleware
- **UI Framework**: shadcn/ui initialized (Button, Input, Form components)
- **Security**: dotenvx for encrypted environment variables
